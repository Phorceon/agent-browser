    ...
    while (steps < MAX_STEPS && !this.aborted) {
      ...
      if (!toolCalls || toolCalls.length === 0) {
        if (!text || text.trim() === '') {
          ...
          // Process tool calls using common method
          await this._processToolCalls(retryToolCalls);
      ...
      // Execute tool calls using common method
      await this._processToolCalls(toolCalls);
    }

    // If we exit the loop without returning, it means we hit MAX_STEPS or were aborted
    const finalText = this.messages.length > 0 ? this.messages[this.messages.length - 1].content : null;
    return finalText || 'Task completed or aborted.';
  }

  /**
   * Process an array of tool calls: execute each, handle errors, update messages.
