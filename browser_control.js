...
async function launchBrowser() {
  // Guard: verify paths exist before launching
  if (!fs.existsSync(CHROME_EXECUTABLE)) {
    throw new Error(`Chrome executable not found at: ${CHROME_EXECUTABLE}. Please verify Chrome is installed.`);
  }
  if (!fs.existsSync(CHROME_PROFILE_PATH)) {
    throw new Error(`Chrome profile path not found at: ${CHROME_PROFILE_PATH}. Please check your profile settings.`);
  }
